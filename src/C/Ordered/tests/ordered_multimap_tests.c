...
int main(void)
{
    ...
    tds_ordered_policy_init(&policy, &int_type, hash_int, equal_int, NULL);
    tds_ordered_multimap map;
    CHECK_STATUS(tds_ordered_multimap_init(&map, &policy, &policy));

    const int b = 2, a = 1, c = 3;
    const int two = 20, nine = 90, one = 10, seven = 70, eight = 80;
    tds_ordered_multimap next;
#define ADD(key, value) do { \
    CHECK_STATUS(tds_ordered_multimap_add(&map, &(key), &(value), &next)); \
    tds_ordered_multimap_destroy(&map); \
    tds_ordered_multimap_move(&map, &next); \
} while (0)
    ADD(b, two);
    ADD(a, nine);
    ADD(b, one);
...
#undef ADD
    ...
    CHECK(tds_ordered_multimap_key_count(&map) == 3u);
    CHECK(tds_ordered_multimap_pair_count(&map) == 5);
    visit_state visited = {{{0}}, 0u};
    CHECK_STATUS(tds_ordered_multimap_visit(&map, record_pair, &visited));
    ...
    CHECK(tds_ordered_multimap_debug_validate(&map));

    CHECK_STATUS(tds_ordered_multimap_add(&map, &b, &two, &next));
    CHECK(tds_ordered_multimap_debug_shares_groups(&map, &next));
    tds_ordered_multimap_destroy(&next);

    tds_ordered_multimap snapshot;
    CHECK_STATUS(tds_ordered_multimap_clone(&map, &snapshot));
    CHECK_STATUS(tds_ordered_multimap_remove(&map, &b, &two, &next));
    tds_ordered_multimap_destroy(&map);
    tds_ordered_multimap_move(&map, &next);
    CHECK_STATUS(tds_ordered_multimap_remove(&map, &b, &one, &next));
    tds_ordered_multimap_destroy(&map);
    tds_ordered_multimap_move(&map, &next);
    CHECK(!tds_ordered_multimap_contains_key(&map, &b));
    CHECK(tds_ordered_multimap_contains(&snapshot, &b, &two));
    CHECK_STATUS(tds_ordered_multimap_add(&map, &b, &two, &next));
    tds_ordered_multimap_destroy(&map);
    tds_ordered_multimap_move(&map, &next);
    visited.count = 0u;
