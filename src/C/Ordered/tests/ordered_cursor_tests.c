...
#define REQUIRE_STATUS(expression) REQUIRE((expression) == TDS_ORDERED_OK)
...
static void init_int_policy(tds_ordered_policy* policy)
{
    ...
    tds_ordered_policy_init(policy, &type, hash_int, equal_int, NULL);
}

static void init_int_map_policy(tds_ordered_map_policy* policy)
{
    ft_value_type type;
    ft_value_type_init(&type, sizeof(int));
...
{
    if (tds_ordered_set_size(set) != count) {
        return false;
    }
...
static bool map_equals(
    ...
    size_t count)
{
    if (tds_ordered_map_size(map) != count) {
        return false;
    }
    for (size_t index = 0u; index != count; ++index) {
        const void* key = NULL;
        const void* value = NULL;
        if (tds_ordered_map_entry_at(map, index, &key, &value) != TDS_ORDERED_OK
    ...
    size_t count)
{
    ...
    return tds_ordered_multimap_visit(map, collect_pair, &buffer) == TDS_ORDERED_OK
        && buffer.count == count
...
static void test_ordered_set_cursor(void)
{
    ...
    tds_ordered_set source;
    REQUIRE_STATUS(tds_ordered_set_from_array(&source, &policy, items, 3u));

    tds_ordered_set_cursor cursor;
    REQUIRE_STATUS(tds_ordered_set_get_cursor(&source, 1u, &cursor));
    bool found = false;
    const void* item = NULL;
    REQUIRE_STATUS(tds_ordered_set_cursor_try_peek_previous(&cursor, &found, &item));
    REQUIRE(found && *(const int*)item == 1);
    REQUIRE_STATUS(tds_ordered_set_cursor_try_peek_next(&cursor, &found, &item));
    REQUIRE(found && *(const int*)item == 2);

    const int inserted_item = 9;
    tds_ordered_set_cursor inserted;
    REQUIRE_STATUS(tds_ordered_set_cursor_insert(&cursor, &inserted_item, &inserted));
    REQUIRE(tds_ordered_set_cursor_position(&inserted) == 2u);
    const int inserted_expected[] = { 1, 9, 2, 3 };
    tds_ordered_set snapshot;
    REQUIRE_STATUS(tds_ordered_set_cursor_snapshot(&inserted, &snapshot));
    REQUIRE(set_equals(&snapshot, inserted_expected, 4u));
    tds_ordered_set_destroy(&snapshot);

    const int duplicate_item = 2;
    bool was_inserted = true;
    tds_ordered_set_cursor duplicate;
    REQUIRE_STATUS(tds_ordered_set_cursor_try_insert(
        &inserted, &duplicate_item, &was_inserted, &duplicate));
    REQUIRE(!was_inserted && tds_ordered_set_cursor_position(&duplicate) == 2u);
    REQUIRE(tds_ordered_set_debug_shares_index(&inserted.set, &duplicate.set));

    tds_ordered_set_cursor deleted;
    REQUIRE_STATUS(tds_ordered_set_cursor_delete_previous(&inserted, &deleted));
    REQUIRE(tds_ordered_set_cursor_position(&deleted) == 1u);
    REQUIRE_STATUS(tds_ordered_set_cursor_snapshot(&deleted, &snapshot));
    REQUIRE(set_equals(&snapshot, items, 3u));
    tds_ordered_set_destroy(&snapshot);

    tds_ordered_set_cursor_destroy(&deleted);
    REQUIRE_STATUS(tds_ordered_set_get_cursor_at_item(
        &source, &duplicate_item, &found, &deleted));
    REQUIRE(found && tds_ordered_set_cursor_position(&deleted) == 1u);
    REQUIRE_STATUS(tds_ordered_set_cursor_seek(&deleted, 2u, &deleted));
    REQUIRE(tds_ordered_set_cursor_position(&deleted) == 2u);

    tds_ordered_set_cursor_destroy(&deleted);
    tds_ordered_set_cursor_destroy(&duplicate);
    tds_ordered_set_cursor_destroy(&inserted);
    tds_ordered_set_cursor_destroy(&cursor);
    tds_ordered_set_destroy(&source);
}

static void replace_map(tds_ordered_map* current, tds_ordered_map* next)
{
    tds_ordered_map_destroy(current);
    tds_ordered_map_move(current, next);
}
...
static void test_ordered_map_cursor(void)
{
    ...
    REQUIRE_STATUS(tds_ordered_map_init(&source, &policy));
    for (int key = 1; key <= 3; ++key) {
        const int value = key;
        tds_ordered_map next;
        REQUIRE_STATUS(tds_ordered_map_add(&source, &key, &value, &next));
        replace_map(&source, &next);
    }

    tds_ordered_map_cursor cursor;
    REQUIRE_STATUS(tds_ordered_map_get_cursor(&source, 1u, &cursor));
    const int key = 9;
    const int value = 90;
    tds_ordered_map_cursor inserted;
    REQUIRE_STATUS(tds_ordered_map_cursor_insert(&cursor, &key, &value, &inserted));
    const int updated_value = 20;
    tds_ordered_map_cursor updated;
    REQUIRE_STATUS(tds_ordered_map_cursor_set_next_value(
        &inserted, &updated_value, &updated));
    const int edit_keys[] = { 1, 9, 2, 3 };
    const int edit_values[] = { 1, 90, 20, 3 };
    tds_ordered_map snapshot;
    REQUIRE_STATUS(tds_ordered_map_cursor_snapshot(&updated, &snapshot));
    ...
    bool was_inserted = true;
    tds_ordered_map_cursor duplicate;
    REQUIRE_STATUS(tds_ordered_map_cursor_try_insert(
        &updated, &duplicate_key, &ignored_value, &was_inserted, &duplicate));
    REQUIRE(!was_inserted && tds_ordered_map_cursor_position(&duplicate) == 2u);

    tds_ordered_map_cursor without_previous;
    REQUIRE_STATUS(tds_ordered_map_cursor_delete_previous(
        &updated, &without_previous));
    tds_ordered_map_cursor deleted;
    REQUIRE_STATUS(tds_ordered_map_cursor_delete_next(
        &without_previous, &deleted));
    const int deleted_keys[] = { 1, 3 };
    const int deleted_values[] = { 1, 3 };
    REQUIRE_STATUS(tds_ordered_map_cursor_snapshot(&deleted, &snapshot));
    ...
    tds_ordered_map_cursor_destroy(&deleted);
    tds_ordered_map_cursor_destroy(&without_previous);
    tds_ordered_map_cursor_destroy(&duplicate);
    tds_ordered_map_cursor_destroy(&updated);
    tds_ordered_map_cursor_destroy(&inserted);
    tds_ordered_map_cursor_destroy(&cursor);
    tds_ordered_map_destroy(&source);
}

static void replace_multimap(
    tds_ordered_multimap* current,
    tds_ordered_multimap* next)
{
    tds_ordered_multimap_destroy(current);
    tds_ordered_multimap_move(current, next);
}
...
static void test_ordered_multimap_cursor(void)
{
    ...
    REQUIRE_STATUS(tds_ordered_multimap_init(&source, &policy, &policy));
    ...
    REQUIRE_STATUS(tds_ordered_multimap_get_cursor_at_pair(
        &source, &find_key, &find_value, &found, &cursor));
    REQUIRE(found && tds_ordered_multimap_cursor_position(&cursor) == 1);
    ...
    bool was_inserted = true;
    tds_ordered_multimap_cursor duplicate;
    REQUIRE_STATUS(tds_ordered_multimap_cursor_try_add(
        &added, &find_key, &added_value, &was_inserted, &duplicate));
    REQUIRE(!was_inserted && tds_ordered_multimap_cursor_position(&duplicate) == 3);
    REQUIRE(tds_ordered_multimap_debug_shares_groups(&added.map, &duplicate.map));

    tds_ordered_multimap_cursor without_previous;
    REQUIRE_STATUS(tds_ordered_multimap_cursor_delete_previous(
        &added, &without_previous));
    tds_ordered_multimap_cursor deleted;
    REQUIRE_STATUS(tds_ordered_multimap_cursor_delete_next(
        &without_previous, &deleted));
    const int deleted_keys[] = { 2, 2, 3 };
    const int deleted_values[] = { 2, 1, 7 };
    tds_ordered_multimap snapshot;
    REQUIRE_STATUS(tds_ordered_multimap_cursor_snapshot(&deleted, &snapshot));
    ...
    tds_ordered_multimap_cursor group;
    REQUIRE_STATUS(tds_ordered_multimap_get_cursor_at_group(
        &source, &group_key, &found, &group));
    REQUIRE(found && tds_ordered_multimap_cursor_position(&group) == 2);
    const void* next_key = NULL;
    const void* next_value = NULL;
    REQUIRE_STATUS(tds_ordered_multimap_cursor_try_peek_next(
    ...
    tds_ordered_multimap_cursor_destroy(&group);
    tds_ordered_multimap_cursor_destroy(&deleted);
    tds_ordered_multimap_cursor_destroy(&without_previous);
    tds_ordered_multimap_cursor_destroy(&duplicate);
