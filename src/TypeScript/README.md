...
| Import | Main types |
| --- | --- |
| `@vladimir-reshetnikov/data-structures/hamt` | `PersistentHashMap`, `PersistentHashSet`, their single-owner transients, `ConcurrentHashTrie`, 32/64-bit Patricia maps and sets, the exact-wire `MerkleSearchTree`, codecs, stores, packs, finite verification budgets, `MSP2` proofs, synchronization, and typed merge |
...
The root import re-exports all four families. See [API and semantic notes](docs/api-notes.md) for
language-specific mappings and deliberately different performance claims.
