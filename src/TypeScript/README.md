...
| Import | Main types |
| --- | --- |
| `@vladimir-reshetnikov/data-structures/hamt` | `PersistentHashMap` with one-descent factory updates, `PersistentHashSet`, `PersistentHashBag`, strict `PersistentBiMap`, reusable `HashMapBulkBuilder`, map/set single-owner transients, the isolate-local snapshotting `ConcurrentHashTrie` facade, 32/64-bit Patricia maps and sets, the exact-wire `MerkleSearchTree`, codecs, stores, packs, finite verification budgets, `MSP2` proofs, synchronization, and typed merge |
