#include <tools/data_structures/wolfram/wolfram.hpp>
...
namespace wf = tools::data_structures::wolfram;
...
{
    tests.add("persistent list constructs and preserves Wolfram-style order", [] {
...
void add_persistent_association_tests(suite& tests)
{
    ...
    tests.add("persistent association SetItem Append Prepend and Join follow Wolfram ordering rules", [] {
...
int main()
{
    ...
    tests.add("aggregate header exposes version metadata", [] {
        FT_REQUIRE_EQUAL(wf::library_name, std::string_view{"Tools.DataStructures.Wolfram.Cpp"});
