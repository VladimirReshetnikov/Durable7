...
- Scope: Non-obvious C#→C++ porting hazards found by reviewing the original `Durable7.FingerTree`
...
library under [`FingerTree/src/Durable7.FingerTree`](../../../CSharp/src/Durable7.FingerTree)
...
1. The **middle subtree** — `MeasuredMiddle` / `LazyMeasuredMiddle._state`, published with
   ...
   ([`MeasuredMiddle.cs:101-110`](../../../CSharp/src/Durable7.FingerTree/Internal/Measured/MeasuredMiddle.cs)).
2. The **deep node's own combined measure** — `DeepMeasuredTree._measureBox`, a *separate* field, published with a
   *separate* `Interlocked.CompareExchange`
   ([`MeasuredTreeLevels.cs:165-197`](../../../CSharp/src/Durable7.FingerTree/Internal/Measured/MeasuredTreeLevels.cs)).
...
([`MeasuredTreeLevels.cs:165-166`](../../../CSharp/src/Durable7.FingerTree/Internal/Measured/MeasuredTreeLevels.cs)).
...
([`TearableConcurrencyStressTests.cs:242-262`](../../../CSharp/tests/Durable7.FingerTree.Tests/TearableConcurrencyStressTests.cs)).
...
([`MeasuredMiddle.cs:103,108`](../../../CSharp/src/Durable7.FingerTree/Internal/Measured/MeasuredMiddle.cs);
[`MeasuredTreeLevels.cs:187,194`](../../../CSharp/src/Durable7.FingerTree/Internal/Measured/MeasuredTreeLevels.cs)).
...
([`RopeChunk.cs:42-80`](../../../CSharp/src/Durable7.FingerTree/Internal/RopeChunk.cs), remark at
...
([`MeasuredMiddle.cs:117-129,184-188,214-218,248-252,280-284`](../../../CSharp/src/Durable7.FingerTree/Internal/Measured/MeasuredMiddle.cs)).
...
([`FingerTree.cs:27-37`](../../../CSharp/src/Durable7.FingerTree/FingerTree.cs)).
...
([`MiddleTree.cs:77`](../../../CSharp/src/Durable7.FingerTree/Internal/MiddleTree.cs);
[`MeasuredMiddle.cs:108`](../../../CSharp/src/Durable7.FingerTree/Internal/Measured/MeasuredMiddle.cs)).
...
([`Tree.cs:34,360-372,378`](../../../CSharp/src/Durable7.FingerTree/Internal/Tree.cs)). Its only
...
([`MeasuredTree.cs:41-42,298-305`](../../../CSharp/src/Durable7.FingerTree/Internal/Measured/MeasuredTree.cs);
[`MeasuredElements.cs:50-57`](../../../CSharp/src/Durable7.FingerTree/Internal/Measured/MeasuredElements.cs)).
For the deque, `Tree<T, TChild>`'s middle is `Tree<T, Node<T, TChild>>`
([`Tree.cs:11-14,344`](../../../CSharp/src/Durable7.FingerTree/Internal/Tree.cs)). Each deeper
...
[`ReversibleTree.cs:386,491,505`](../../../CSharp/src/Durable7.FingerTree/Internal/Reversible/ReversibleTree.cs);
[`ReversibleDeque.cs:145,163`](../../../CSharp/src/Durable7.FingerTree/ReversibleDeque.cs)) become
...
1. **Order-independent measure + runtime comparer in the predicate** — `SortedBag/Set/Dictionary`. The measure is
   ...
   touches the comparer** ([`BuiltInMeasures.cs:158-169`](../../../CSharp/src/Durable7.FingerTree/BuiltInMeasures.cs)).
   ...
   [`SortedSet.cs:45,123,353`](../../../CSharp/src/Durable7.FingerTree/SortedSet.cs)). This is what
...
2. **Comparison baked into the measure monoid** — `PriorityQueue` and `IntervalTree`. Here `Combine` itself calls
   ...
   [`PriorityQueue.cs:36-43`](../../../CSharp/src/Durable7.FingerTree/PriorityQueue.cs)) and the
   interval tree's max-high (`CombineMax` uses `Comparer<T>.Default`,
   [`IntervalTree.cs:73-80`](../../../CSharp/src/Durable7.FingerTree/IntervalTree.cs)). C# is
   explicit that "a min-priority queue's `Combine` depends on the order, so unlike the sorted collections it
   **cannot accept a runtime comparer**" ([`PriorityQueue.cs:60-63`](../../../CSharp/src/Durable7.FingerTree/PriorityQueue.cs)).
...
3. **Static comparison policy** — the `MaxMeasure<T, TComparison>` / `MinMeasure<T, TComparison>` family and the
   ...
   ([`BuiltInMeasures.cs:178-222`](../../../CSharp/src/Durable7.FingerTree/BuiltInMeasures.cs);
   [`Comparisons.cs`](../../../CSharp/src/Durable7.FingerTree/Comparisons.cs)). These map directly
...
([`ReversibleTree.cs:13-17`](../../../CSharp/src/Durable7.FingerTree/Internal/Reversible/ReversibleTree.cs);
[`ReversibleDeque.cs:22`](../../../CSharp/src/Durable7.FingerTree/ReversibleDeque.cs)). Its deep
...
- **O(1) `reverse()` depends on *sharing* children, not deep-copying them.** `Mirror()` builds a new node that
  ...
  ([`ReversibleTree.cs:288`](../../../CSharp/src/Durable7.FingerTree/Internal/Reversible/ReversibleTree.cs);
  [`ReversibleElements.cs:66,130`](../../../CSharp/src/Durable7.FingerTree/Internal/Reversible/ReversibleElements.cs)).
...
- **Orientation-aware accessors allocate on the reversed path.** `LogicalChild`/`LogicalChildren` under reversal
  ...
  ([`ReversibleElements.cs:134-144`](../../../CSharp/src/Durable7.FingerTree/Internal/Reversible/ReversibleElements.cs)).
...
- **Endpoint reads use an opposite-digit identity, not `Mirror`.** Reversed `First`/`Last` read cached
  ...
  ([`ReversibleTree.cs:273-276`](../../../CSharp/src/Durable7.FingerTree/Internal/Reversible/ReversibleTree.cs);
  [`ReversibleElements.cs:124-127`](../../../CSharp/src/Durable7.FingerTree/Internal/Reversible/ReversibleElements.cs)).
...
([`RopeChunk.cs:14-37`](../../../CSharp/src/Durable7.FingerTree/Internal/RopeChunk.cs)). The C++
...
- **A one-element slice pins the whole backing buffer** until `compact()` rebuilds it. This is the *reason*
  ...
  an eager copy.
- **`FromChunks` deliberately does not copy** caller memory ([`Rope.cs:347-364`](../../../CSharp/src/Durable7.FingerTree/Rope.cs)).
...
re-walks the slice to recompute it ([`MeasuredRopeChunk.cs:23-27,52,74-81`](../../../CSharp/src/Durable7.FingerTree/Internal/MeasuredRopeChunk.cs)).
...
([`MeasuredRope.cs:480-502`](../../../CSharp/src/Durable7.FingerTree/MeasuredRope.cs)). An
...
`MeasuredRope<char, int, NewlineMeasure>` ([`RopeBuilder.cs:21-83`](../../../CSharp/src/Durable7.FingerTree/RopeBuilder.cs)).
...
arithmetic ([`Measures.cs:81`](../../../CSharp/src/Durable7.FingerTree/Measures.cs)), and the
...
- **Ranks that can be absent need `std::optional<std::size_t>`, not a `-1` sentinel.** `IndexOf`/`IndexOfKey`
  return `-1` when absent ([`SortedSet.cs:122-126`](../../../CSharp/src/Durable7.FingerTree/SortedSet.cs));
...
- **`Try…(out …)` → optional / total result.** The read API is bool-returning `Try` methods with `out` values set
  to `default!` on the false path ([`FingerTree.cs:137-167,259-346`](../../../CSharp/src/Durable7.FingerTree/FingerTree.cs)).
  ...
  nothing matches, [`FingerTree.cs:335-339`](../../../CSharp/src/Durable7.FingerTree/FingerTree.cs)).
...
- **Value carriers need `operator==` (and sometimes `std::hash`); containers must not.** The public
  ...
  equality ([`FingerTreeDeque.cs:38-40`](../../../CSharp/src/Durable7.FingerTree/FingerTreeDeque.cs));
...
- **Indexable types keep `operator[]`/`at`.** `FingerTreeDeque<T>` is `IReadOnlyList<T>` with an O(log min(i+1,
  ...
  ([`FingerTreeDeque.cs:56,112-120`](../../../CSharp/src/Durable7.FingerTree/FingerTreeDeque.cs);
  [`SortedSet.cs:109-117`](../../../CSharp/src/Durable7.FingerTree/SortedSet.cs)). The plan's
...
- **Most of the library iterates by eager materialization, not a lazy walk.** `FingerTree.GetEnumerator` and
  ...
  ([`FingerTree.cs:350-367`](../../../CSharp/src/Durable7.FingerTree/FingerTree.cs)), and the
  ...
  both operands first ([`SortedSet.cs:355-403`](../../../CSharp/src/Durable7.FingerTree/SortedSet.cs))
...
- **Tearable concurrent-first-read test (H-adjacent).** Milestone 3 says only "concurrent first-force read tests."
  ...
  *both element and measure*, with (a) many-thread reads of one immutable tree, (b) concurrent *first* reads of a
  ...
  ([`TearableConcurrencyStressTests.cs`](../../../CSharp/tests/Durable7.FingerTree.Tests/TearableConcurrencyStressTests.cs)).
...
- **Complexity guards** (allocation-flatness, no-force endpoint reads, comparer-call counts, size-independent
  ...
  ([`FingerTreeDequeComplexityGuardTests.cs`](../../../CSharp/tests/Durable7.FingerTree.Tests/FingerTreeDequeComplexityGuardTests.cs);
  [`AllocationFreeReadTests.cs`](../../../CSharp/tests/Durable7.FingerTree.Tests/AllocationFreeReadTests.cs);
  [`ZeroClosureNamedOpTests.cs`](../../../CSharp/tests/Durable7.FingerTree.Tests/ZeroClosureNamedOpTests.cs)).
...
- **`try_locate` vs `try_split_find` equivalence across every threshold** ([`TryLocateTests.cs`](../../../CSharp/tests/Durable7.FingerTree.Tests/TryLocateTests.cs))
...
- **Samples:** C# ships three (Tour, Showcase, Editor). Editor is correctly dropped (text extras). But Tour ≠ the
  ...
  `run(std::ostream&)` seam smoke-tested via CTest ([`SampleSmokeTests.cs`](../../../CSharp/tests/Durable7.FingerTree.Tests/SampleSmokeTests.cs)).
...
([`RopeText.cs:91-174`](../../../CSharp/src/Durable7.FingerTree/RopeText.cs)). The Tour sample
uses exactly those operations for its second act
([`TourProgram.cs:57-67`](../../../CSharp/samples/Durable7.FingerTree.Tour/TourProgram.cs)).
...
([`RopeTextExtras.cs`](../../../CSharp/src/Durable7.FingerTree/RopeTextExtras.cs);
[`RopeText.cs:73-89`](../../../CSharp/src/Durable7.FingerTree/RopeText.cs)). The plan now keeps
...
- **"Concat forces both middles, breaking the lazy bound"** — `Glue` does force both operand middles
  ([`MeasuredTree.cs:139`](../../../CSharp/src/Durable7.FingerTree/Internal/Measured/MeasuredTree.cs)),
