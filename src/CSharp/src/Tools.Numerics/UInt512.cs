    ...
    IMinMaxValue<UInt512>
{
    ...
    {
        UInt256 lo = left._lower + right._lower;
        UInt256 carry = lo < left._lower ? (UInt256)1 : 0;
    ...
    {
        UInt256 lo = left._lower - right._lower;
        UInt256 borrow = lo > left._lower ? (UInt256)1 : 0;
    ...
    private static string FormatDecimal(UInt512 value, IFormatProvider? provider)
    {
        ...
        Span<UInt512> parts = stackalloc UInt512[14];
        int count = 0;
        UInt512 current = value;
        UInt512 divisor = new(0, 10_000_000_000_000_000_000UL);
        while (!current.IsZero)
        {
            Divide(current, divisor, out current, out UInt512 rem);
            parts[count++] = rem;
        }
    ...
    /// <summary>
    /// Divides one unsigned 512-bit value by another using restoring binary long division.
    /// </summary>
