...
using static Durable7.Numerics.BitHelpers;

namespace Durable7.Numerics;

/// <summary>
/// Represents a signed 256-bit integer that follows two's-complement semantics.
/// </summary>
/// <remarks>
/// <para>
/// <see cref="Int256"/> is designed as the signed companion to <see cref="UInt256"/> and intentionally mirrors the
...
/// </para>
/// <para>
/// Storage uses two <see cref="UInt128"/> halves in little-endian-by-halves order: <c>_lower</c> carries bits 0..127
/// and <c>_upper</c> carries bits 128..255, including the sign bit at position 255. Checked operators enforce the
/// inclusive signed range <c>[-2^255, 2^255 - 1]</c>, while unchecked operators wrap modulo <c>2^256</c> and
/// reinterpret the resulting bit pattern as signed.
/// </para>
/// <para>
/// Arithmetic operations are implemented over the two 128-bit halves (including explicit multi-limb
...
/// counterpart <see cref="UInt256"/> (in this project) to preserve behavioral consistency.
/// </para>
/// </remarks>
    ...
    IMinMaxValue<Int256>
{
    ...
    private const int MagnitudeBits = 255;
    ...
    /// <summary>The most significant 128 bits, including the sign bit at bit position 255.</summary>
    private readonly UInt128 _upper;

    /// <summary>The least significant 128 bits of the value.</summary>
    private readonly UInt128 _lower;

    /// <summary>Bit mask that isolates the sign bit within the upper 128-bit half.</summary>
    private static readonly UInt128 s_signMaskUpper = UInt128.One << 127;

    /// <summary>Initializes a new <see cref="Int256"/> from its upper and lower 128-bit halves.</summary>
    /// <param name="upper">The most significant 128 bits in two's-complement form.</param>
    /// <param name="lower">The least significant 128 bits.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Int256(UInt128 upper, UInt128 lower) => (_upper, _lower) = (upper, lower);

    /// <summary>Initializes a new <see cref="Int256"/> from a 64-bit signed integer.</summary>
    ...
    public Int256(long value)
    {
        Int128 v = value;
        _lower = (UInt128)v;
        _upper = v < 0 ? UInt128.MaxValue : 0;
    }

    /// <summary>Gets the upper 128-bit half (including the sign bit).</summary>
    internal UInt128 Upper => _upper;

    /// <summary>Gets the lower 128-bit half.</summary>
    ...
    public static readonly Int256 One = new(0, 1);

    /// <summary>Represents the smallest possible <see cref="Int256"/> value.</summary>
    public static readonly Int256 MinValue = new(UInt128.One << 127, 0);

    /// <summary>Represents the largest possible <see cref="Int256"/> value.</summary>
    public static readonly Int256 MaxValue = new((UInt128)Int128.MaxValue, UInt128.MaxValue);

    static Int256 IMinMaxValue<Int256>.MinValue => MinValue;

    static Int256 IMinMaxValue<Int256>.MaxValue => MaxValue;

    static Int256 IParsable<Int256>.Parse(string s, IFormatProvider? provider) => Parse(s, provider);
    ...
    /// <summary>Compares the current value with another signed 256-bit value.</summary>
    ...
    {
        int c = ((Int128)_upper).CompareTo((Int128)other._upper);
    ...
    /// </returns>
    /// <exception cref="ArgumentException"><paramref name="obj"/> is not an <see cref="Int256"/>.</exception>
    int IComparable.CompareTo(object? obj) =>
        obj switch
        {
            null => 1,
            Int256 other => CompareTo(other),
            _ => throw new ArgumentException("Object must be an Int256.")
        };

    /// <summary>Determines whether the current value equals another <see cref="Int256"/> value.</summary>
    /// <param name="other">The value to compare with this instance.</param>
    /// <returns><see langword="true"/> when both halves match; otherwise, <see langword="false"/>.</returns>
    public bool Equals(Int256 other) => (_upper, _lower) == (other._upper, other._lower);

    /// <inheritdoc/>
    public override bool Equals([NotNullWhen(true)] object? obj) => obj is Int256 other && Equals(other);
        ...
        IFormatProvider? provider = null)
    {
        ...
        UInt256 magnitude = hex ? (UInt256)this : GetUnsignedMagnitude(this);
        return UInt256.TryFormatMagnitude(magnitude, !hex && IsNegative,
    ...
    /// <summary>Converts the string representation of a number to its <see cref="Int256"/> equivalent.</summary>
    /// <param name="s">A string that contains the number to convert.</param>
    /// <returns>The parsed signed 256-bit value represented by <paramref name="s"/>.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="s"/> is <see langword="null"/>.</exception>
    /// <exception cref="FormatException">
    /// <paramref name="s"/> does not represent a valid integer in the default style.
    /// </exception>
    /// <exception cref="OverflowException">
    /// <paramref name="s"/> represents a value outside the inclusive range <c>[-2^255, 2^255 - 1]</c>.
    /// </exception>
    public static Int256 Parse(string s) => Parse(s, NumberStyles.Integer, CultureInfo.CurrentCulture);

    /// <summary>
    /// Converts the string representation of a number to its <see cref="Int256"/> equivalent using the specified format
    /// provider.
    /// </summary>
    /// <param name="s">A string that contains the number to convert.</param>
    ...
    /// <paramref name="s"/> represents a value outside the inclusive range <c>[-2^255, 2^255 - 1]</c>.
    /// </exception>
    public static Int256 Parse(string s, IFormatProvider? provider) => Parse(s, NumberStyles.Integer, provider);

    /// <summary>
    /// Converts the string representation of a number in a specified style and culture-specific format to its <see cref="Int256"/> equivalent.
    /// </summary>
    /// <param name="s">A string that contains the number to convert.</param>
    /// <param name="style">
    ...
    /// <paramref name="s"/> represents a value outside the inclusive range <c>[-2^255, 2^255 - 1]</c>.
    /// </exception>
    public static Int256 Parse(string s, NumberStyles style, IFormatProvider? provider = null)
    {
    ...
    /// <summary>
    /// Converts the span representation of a number in a specified style and culture-specific format to its <see cref="Int256"/> equivalent.
    /// </summary>
    ...
    /// <summary>
    /// Converts the UTF-8 span representation of a number in a specified style and culture-specific format to its <see cref="Int256"/> equivalent.
    /// </summary>
    ...
    /// </param>
    /// <param name="provider">An object that supplies culture-specific parsing information.</param>
    /// <returns>The parsed signed 256-bit value represented by <paramref name="utf8Text"/>.</returns>
    /// <exception cref="ArgumentException">
    /// <paramref name="style"/> contains undefined <see cref="NumberStyles"/> flags, or combines
    /// <see cref="NumberStyles.AllowHexSpecifier"/> or <see cref="NumberStyles.AllowBinarySpecifier"/> with flags
    /// outside <see cref="NumberStyles.HexNumber"/> or <see cref="NumberStyles.BinaryNumber"/> respectively.
    ...
    /// </exception>
    /// <exception cref="OverflowException">
    /// <paramref name="utf8Text"/> represents a value outside the inclusive range <c>[-2^255, 2^255 - 1]</c>.
    /// </exception>
    public static Int256 Parse(
    ...
    /// <summary>
    /// Attempts to convert the string representation of a number to its <see cref="Int256"/> equivalent.
    /// </summary>
    /// <param name="s">A string that contains the number to convert.</param>
    /// <param name="result">
    /// When this method returns <see langword="true"/>, contains the parsed signed 256-bit value represented by
    ...
    /// its <see cref="Int256"/> equivalent.
    /// </summary>
    /// <param name="s">A string that contains the number to convert.</param>
    /// <param name="style">
    /// A bitwise combination of <see cref="NumberStyles"/> values that specifies permitted syntactic elements.
    ...
    /// <summary>
    /// Attempts to convert the span representation of a number to its <see cref="Int256"/> equivalent.
    /// </summary>
    /// <param name="s">A character span that contains the number to convert.</param>
    /// <param name="result">
    /// When this method returns <see langword="true"/>, contains the parsed signed 256-bit value represented by
    ...
    public static bool TryParse(
        ...
        NumberStyles style,
        IFormatProvider? provider,
        out Int256 result) => TryParseCore(s, style, provider, out result);

    /// <summary>
    /// Attempts to convert the UTF-8 span representation of a number to its <see cref="Int256"/> equivalent.
    /// </summary>
    /// <param name="utf8Text">A UTF-8 byte span that contains the number to convert.</param>
    /// <param name="result">
    /// When this method returns <see langword="true"/>, contains the parsed signed 256-bit value represented by
    ...
    /// <see langword="true"/> if parsing succeeds and the value is within range; otherwise, <see langword="false"/>.
    /// </returns>
    public static bool TryParse(ReadOnlySpan<byte> utf8Text, out Int256 result)
    ...
    /// to its <see cref="Int256"/> equivalent.
    /// </summary>
    /// <param name="utf8Text">A UTF-8 byte span that contains the number to convert.</param>
    /// <param name="style">
    /// A bitwise combination of <see cref="NumberStyles"/> values that specifies permitted syntactic elements.
    ...
    /// <summary>Adds two signed 256-bit values.</summary>
    ...
    public static Int256 operator +(Int256 left, Int256 right)
    {
        ...
        return new(left._upper + right._upper + (lo < left._lower ? UInt128.One : UInt128.Zero), lo);
    }

    /// <summary>Subtracts one signed 256-bit value from another.</summary>
    ...
    public static Int256 operator -(Int256 left, Int256 right) =>
        new(
            left._upper - right._upper - (left._lower < right._lower ? UInt128.One : UInt128.Zero),
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Int256 operator ~(Int256 value) => new(~value._upper, ~value._lower);
    ...
    {
        shiftAmount &= 0xFF;
        Int128 upperSigned = (Int128)value._upper;
        UInt128 fill = upperSigned < 0 ? UInt128.MaxValue : 0;

        return shiftAmount switch
        {
            0 => value,
            < 128 => new(
                (UInt128)(upperSigned >> shiftAmount),
                (value._lower >> shiftAmount) | (value._upper << (128 - shiftAmount))),
            128 => new(fill, (UInt128)upperSigned),
            _ => new(fill, (UInt128)(upperSigned >> (shiftAmount - 128)))
        };
    }
    ...
    /// <param name="shiftAmount">The number of bit positions to shift.</param>
    /// <remarks>
    /// This operator performs a logical shift, unlike <see cref="operator >>(Int256, int)"/> which preserves the
    /// sign bit through arithmetic extension.
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Int256 operator >>>(Int256 value, int shiftAmount)
        => (Int256)((UInt256)value >> shiftAmount);

    /// <summary>Multiplies two signed 256-bit values.</summary>
    ...
    /// <summary>Divides one signed 256-bit value by another.</summary>
    /// <param name="left">The dividend.</param>
    /// <param name="right">The divisor.</param>
    public static Int256 operator /(Int256 left, Int256 right)
    {
        Divide(left, right, out Int256 quotient, out _);
        return quotient;
    }

    /// <summary>Computes the remainder after dividing one signed 256-bit value by another.</summary>
    ...
    public static Int256 operator %(Int256 left, Int256 right)
    {
        Divide(left, right, out _, out Int256 remainder);
        return remainder;
    }
    ...
    /// <summary>Adds two values and throws when the result is outside the <see cref="Int256"/> range.</summary>
    /// <param name="left">The augend (the value to which <paramref name="right"/> is added).</param>
    /// <param name="right">The addend (the value added to <paramref name="left"/>).</param>
    /// <exception cref="OverflowException">The result cannot be represented as <see cref="Int256"/>.</exception>
    public static Int256 operator checked +(Int256 left, Int256 right)
    {
        Int256 sum = left + right;
        return (Int128)((sum._upper ^ left._upper) & ~(left._upper ^ right._upper)) < 0
            ? throw new OverflowException()
            : sum;
    }

    /// <summary>Subtracts two values and throws when the result is outside the <see cref="Int256"/> range.</summary>
    /// <param name="left">The minuend (the value from which <paramref name="right"/> is subtracted).</param>
    /// <param name="right">The subtrahend (the value subtracted from <paramref name="left"/>).</param>
    /// <exception cref="OverflowException">The result cannot be represented as <see cref="Int256"/>.</exception>
    public static Int256 operator checked -(Int256 left, Int256 right)
    {
        Int256 diff = left - right;
        return (Int128)((diff._upper ^ left._upper) & (left._upper ^ right._upper)) < 0
            ? throw new OverflowException()
            : diff;
    }

    /// <summary>Multiplies two values and throws when the product is outside the <see cref="Int256"/> range.</summary>
    /// <param name="left">The multiplicand.</param>
    /// <param name="right">The multiplier.</param>
    /// <exception cref="OverflowException">The product cannot be represented as <see cref="Int256"/>.</exception>
    public static Int256 operator checked *(Int256 left, Int256 right)
    {
    ...
    /// <param name="value">The operand value for the operation.</param>
    public static int LeadingZeroCount(Int256 value) =>
        value._upper == 0
    ...
    /// <param name="value">The operand value for the operation.</param>
    public static int TrailingZeroCount(Int256 value) =>
        value._lower == 0
    ...
    /// <param name="value">The operand value for the operation.</param>
    public static int PopCount(Int256 value)
    ...
    /// <returns>
    /// A <see cref="UInt256"/> that represents <c>|value|</c> interpreted in the unsigned 256-bit domain.
    /// </returns>
    private static UInt256 GetUnsignedMagnitude(Int256 value)
    {
        UInt256 bits = new(value._upper, value._lower);
    ...
    /// <summary>
    /// Divides one signed 256-bit value by another and returns both quotient and remainder.
    /// </summary>
    ...
    private static void Divide(Int256 dividend, Int256 divisor, out Int256 quotient, out Int256 remainder)
    {
        ...
        quotient = (Int256)(left / right);
        if (dividend.IsNegative ^ divisor.IsNegative)
            quotient = -quotient;

        remainder = (Int256)(left % right);
        if (dividend.IsNegative)
            remainder = -remainder;
    }

    /// <summary>
    /// Produces the full signed product of two values and returns the upper 256 bits.
    /// </summary>
    /// <param name="left">The first multiplicand.</param>
    /// <param name="right">The second multiplicand.</param>
    /// <param name="lower">Receives the low 256 bits of the product.</param>
    /// <returns>The high 256 bits of the 512-bit signed product.</returns>
    private static Int256 BigMul(Int256 left, Int256 right, out Int256 lower)
    {
        UInt256 upper = BigMulUnsigned((UInt256)left, (UInt256)right, out UInt256 uLower);
        lower = (Int256)uLower;
        return (Int256)upper - ((left >> (Bits - 1)) & right) - ((right >> (Bits - 1)) & left);
    }
    ...
    /// <param name="lower">Receives the low 256 bits of the product.</param>
    /// <returns>The high 256 bits of the 512-bit product.</returns>
    private static UInt256 BigMulUnsigned(UInt256 left, UInt256 right, out UInt256 lower)
    {
        Span<ulong> a =
        [
            (ulong)left.Lower,
        ...
        lower = new(
            ((UInt128)product[3] << 64) | product[2],
            ((UInt128)product[1] << 64) | product[0]);

        return new(
            ((UInt128)product[7] << 64) | product[6],
            ((UInt128)product[5] << 64) | product[4]);
    }
    ...
    /// <summary>
    /// Formats a signed 256-bit value using a supported standard numeric specifier and optional precision.
    /// </summary>
    ...
    /// </exception>
    private static string FormatValue(Int256 value, string? format, IFormatProvider? provider) =>
        string.IsNullOrEmpty(format)
            ...
            : format[0] switch
            {
                ...
                'X' or 'x' => ((UInt256)value).ToString(format, provider),
                _ => throw new FormatException()
            };

    /// <summary>
    /// Formats a signed 256-bit value as base-10 digits.
    /// </summary>
    /// <param name="value">The value to format.</param>
    /// <param name="provider">
    /// The format provider used for culture-specific decimal sign and digit formatting.
    ...
    private static string FormatDecimal(Int256 value, IFormatProvider? provider)
    {
        NumberFormatInfo nfi = NumberFormatInfo.GetInstance(provider);

        if (!value.IsNegative)
            return ((UInt256)value).ToString(null, provider);

        UInt256 magnitude = value == MinValue ? (UInt256)MinValue : (UInt256)(-value);
        return nfi.NegativeSign + magnitude.ToString(null, provider);
    }

    /// <summary>
    /// Parses text into a signed 256-bit value and throws when parsing fails.
    /// </summary>
    ...
    /// </exception>
    /// <exception cref="OverflowException">
    /// Thrown when <paramref name="text"/> represents a numeric value outside the <see cref="Int256"/> range.
    /// </exception>
    private static Int256 ParseCore(ReadOnlySpan<char> text, NumberStyles style, IFormatProvider? provider) =>
        TryParseCore(text, style, provider, out Int256 value, out bool overflow)
            ? value
            : throw (overflow
    ...
    /// <summary>
    /// Attempts to parse text into a signed 256-bit value.
    /// </summary>
    /// <param name="text">The input text to parse.</param>
    /// <param name="style">The accepted style flags.</param>
    /// <param name="provider">
    ...
    private static bool TryParseCore(ReadOnlySpan<char> text, NumberStyles style, IFormatProvider? provider, out Int256 value)
        => TryParseCore(text, style, provider, out value, out _);

    /// <summary>
    /// Attempts to parse text into a signed 256-bit value and reports overflow separately from syntax failures.
    /// </summary>
    /// <param name="text">The input text to parse.</param>
    /// <param name="style">The accepted style flags.</param>
    /// <param name="provider">
    ...
    /// <summary>
    /// Attempts to parse a hexadecimal representation into a signed 256-bit value.
    /// </summary>
    ...
    /// </param>
    /// <returns><see langword="true"/> when parsing succeeds; otherwise, <see langword="false"/>.</returns>
    private static bool TryParseHex(ReadOnlySpan<char> text, NumberStyles style, out Int256 value, out bool overflow)
    {
        value = Zero;
        overflow = false;
    ...
    /// <summary>Converts a <see cref="Int256"/> value to a <see cref="BigInteger"/>.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static explicit operator BigInteger(Int256 value) => value.ToBigInteger();

    /// <summary>
    /// Converts a <see cref="BigInteger"/> to <see cref="Int256"/> using unchecked wraparound semantics modulo 2^256.
    /// </summary>
    /// <param name="value">The operand value for the operation.</param>
    public static explicit operator Int256(BigInteger value)
    {
        ...
        // If src shorter than 32 and negative, we must sign-extend.
    ...
    /// <summary>
    /// Converts a <see cref="BigInteger"/> to <see cref="Int256"/> and throws when the source value is outside the
    /// representable range.
    /// </summary>
    /// <param name="value">The operand value for the operation.</param>
    /// <exception cref="OverflowException">
    /// <paramref name="value"/> is outside the <see cref="Int256"/> range.
    /// </exception>
    public static explicit operator checked Int256(BigInteger value)
    {
        ...
        if (src.Length > Bytes)
        {
            ...
            // it must either fit in 32 or be a redundant sign extension.
    ...
    /// <summary>Converts an 8-bit signed value to <see cref="Int256"/>.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static implicit operator Int256(sbyte value) => new(value);

    /// <summary>Converts a 16-bit signed value to <see cref="Int256"/>.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static implicit operator Int256(short value) => new(value);

    /// <summary>Converts a 32-bit signed value to <see cref="Int256"/>.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static implicit operator Int256(int value) => new(value);

    /// <summary>Converts a 64-bit signed value to <see cref="Int256"/>.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static implicit operator Int256(long value) => new(value);

    /// <summary>Converts an 8-bit unsigned value to <see cref="Int256"/>.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static implicit operator Int256(byte value) => new(0, value);

    /// <summary>Converts a 16-bit unsigned value to <see cref="Int256"/>.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static implicit operator Int256(ushort value) => new(0, value);

    /// <summary>Converts a 32-bit unsigned value to <see cref="Int256"/>.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static implicit operator Int256(uint value) => new(0, value);

    /// <summary>Converts a 64-bit unsigned value to <see cref="Int256"/>.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static implicit operator Int256(ulong value) => new(0, value);

    /// <summary>Converts a 128-bit signed value to <see cref="Int256"/>.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static implicit operator Int256(Int128 value) => new(value < 0 ? UInt128.MaxValue : 0, (UInt128)value);

    /// <summary>Converts a 128-bit unsigned value to <see cref="Int256"/>.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static implicit operator Int256(UInt128 value) => new(0, value);

    /// <summary>Converts an unsigned 256-bit value to <see cref="Int256"/> by reinterpreting its bit pattern.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static explicit operator Int256(UInt256 value) => new(value.Upper, value.Lower);

    /// <summary>Converts an unsigned 256-bit value to <see cref="Int256"/> in a checked context.</summary>
    /// <param name="value">The operand value for the operation.</param>
    ...
    public static explicit operator checked Int256(UInt256 value) =>
        value.Upper > (UInt128)Int128.MaxValue
            ? throw new OverflowException()
            : new(value.Upper, value.Lower);

    /// <summary>Converts a <see cref="Int256"/> value to an 8-bit unsigned integer.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static explicit operator byte(Int256 value) => (byte)value._lower;

    /// <summary>Converts a <see cref="Int256"/> value to an 8-bit unsigned integer in a checked context.</summary>
    /// <param name="value">The operand value for the operation.</param>
    /// <exception cref="OverflowException">The value is outside the range of <see cref="byte"/>.</exception>
    public static explicit operator checked byte(Int256 value) => checked((byte)(ulong)value);

    /// <summary>Converts a <see cref="Int256"/> value to a 16-bit unsigned integer.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static explicit operator ushort(Int256 value) => (ushort)value._lower;

    /// <summary>Converts a <see cref="Int256"/> value to a 16-bit unsigned integer in a checked context.</summary>
    /// <param name="value">The operand value for the operation.</param>
    /// <exception cref="OverflowException">The value is outside the range of <see cref="ushort"/>.</exception>
    public static explicit operator checked ushort(Int256 value) => checked((ushort)(ulong)value);

    /// <summary>Converts a <see cref="Int256"/> value to a 32-bit unsigned integer.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static explicit operator uint(Int256 value) => (uint)value._lower;

    /// <summary>Converts a <see cref="Int256"/> value to a 32-bit unsigned integer in a checked context.</summary>
    /// <param name="value">The operand value for the operation.</param>
    /// <exception cref="OverflowException">The value is outside the range of <see cref="uint"/>.</exception>
    public static explicit operator checked uint(Int256 value) => checked((uint)(ulong)value);

    /// <summary>Converts a <see cref="Int256"/> value to a 64-bit unsigned integer.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static explicit operator ulong(Int256 value) => (ulong)value._lower;

    /// <summary>Converts a <see cref="Int256"/> value to a 64-bit unsigned integer in a checked context.</summary>
    /// <param name="value">The operand value for the operation.</param>
    ...
    /// <summary>
    /// Converts a <see cref="Int256"/> value to a 128-bit unsigned integer by returning the low 128 bits.
    /// </summary>
    /// <param name="value">The operand value for the operation.</param>
    public static explicit operator UInt128(Int256 value) => value._lower;

    /// <summary>Converts a <see cref="Int256"/> value to a 128-bit unsigned integer in a checked context.</summary>
    /// <param name="value">The operand value for the operation.</param>
    ...
    /// <summary>Converts a <see cref="Int256"/> value to an 8-bit signed integer.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static explicit operator sbyte(Int256 value) => (sbyte)value._lower;

    /// <summary>Converts a <see cref="Int256"/> value to an 8-bit signed integer in a checked context.</summary>
    /// <param name="value">The operand value for the operation.</param>
    /// <exception cref="OverflowException">The value is outside the range of <see cref="sbyte"/>.</exception>
    public static explicit operator checked sbyte(Int256 value) => checked((sbyte)(long)value);

    /// <summary>Converts a <see cref="Int256"/> value to a 16-bit signed integer.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static explicit operator short(Int256 value) => (short)value._lower;

    /// <summary>Converts a <see cref="Int256"/> value to a 16-bit signed integer in a checked context.</summary>
    /// <param name="value">The operand value for the operation.</param>
    /// <exception cref="OverflowException">The value is outside the range of <see cref="short"/>.</exception>
    public static explicit operator checked short(Int256 value) => checked((short)(long)value);

    /// <summary>Converts a <see cref="Int256"/> value to a 32-bit signed integer.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static explicit operator int(Int256 value) => (int)value._lower;

    /// <summary>Converts a <see cref="Int256"/> value to a 32-bit signed integer in a checked context.</summary>
    /// <param name="value">The operand value for the operation.</param>
    /// <exception cref="OverflowException">The value is outside the range of <see cref="int"/>.</exception>
    public static explicit operator checked int(Int256 value) => checked((int)(long)value);

    /// <summary>Converts a <see cref="Int256"/> value to a 64-bit signed integer.</summary>
    /// <param name="value">The operand value for the operation.</param>
    public static explicit operator long(Int256 value) => (long)value._lower;

    /// <summary>Converts a <see cref="Int256"/> value to a 64-bit signed integer in a checked context.</summary>
    /// <param name="value">The operand value for the operation.</param>
    ...
    public static explicit operator checked long(Int256 value)
        => checked((long)(Int128)value);

    /// <summary>
    /// Converts a <see cref="Int256"/> value to a 128-bit signed integer by returning the low 128 bits.
    /// </summary>
    /// <param name="value">The operand value for the operation.</param>
    public static explicit operator Int128(Int256 value) => (Int128)value._lower;

    /// <summary>Converts a <see cref="Int256"/> value to a 128-bit signed integer in a checked context.</summary>
    /// <param name="value">The operand value for the operation.</param>
    ...
    public static explicit operator checked Int128(Int256 value)
    {
        ...
        return value._upper == (lower < 0 ? UInt128.MaxValue : 0)
            ? lower
            : throw new OverflowException();
    }
    ...
    /// <summary>
    /// Reads a 256-bit signed integer from a 32-byte little-endian span.
    /// </summary>
    /// <param name="source">A 32-byte source span whose first byte is the least significant byte.</param>
    /// <returns>The decoded <see cref="Int256"/> value.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static Int256 ReadFullLittleEndian(ReadOnlySpan<byte> source) =>
        new(
