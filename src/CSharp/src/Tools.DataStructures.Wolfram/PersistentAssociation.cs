// Persistent insertion-ordered map shaped for Wolfram-Association semantics.
//
...
//
// The target semantics are the kernel-verified Wolfram Association behaviors recorded in the
// Tungsten design study (C:\Smithereens\src\Tungsten\docs\reports\
// 2026-07-03-list-association-persistent-backends.md) and re-verified against Wolfram 15.0
// on 2026-07-07:
//
...
namespace Tools.DataStructures.Wolfram;
...
/// <summary>
/// An immutable, persistent, insertion-ordered key/value map shaped for representing a Wolfram
...
/// </para>
/// <para>
/// Wolfram operation correspondence (indexing is zero-based here, one-based in Wolfram):
...
/// </para>
/// <para>
/// Ordering semantics follow the kernel-verified Wolfram rules: construction keeps a duplicate
    ...
    where TKey : notnull
{
    ...
    /// <summary>
    /// Gets the number of entries. Wolfram: <c>Length</c>.
    /// </summary>
    /// <remarks>O(1).</remarks>
    ...
    /// <summary>
    /// Gets the first entry in association order. Wolfram: <c>First</c> (which returns the value; take <c>.Value</c>).
    /// </summary>
    ...
    /// <summary>
    /// Gets the values in association order. Wolfram: <c>Values</c>.
    /// </summary>
    /// <remarks>O(n) to enumerate, streaming off the order structure without hashing.</remarks>
    ...
    /// <summary>
    /// Gets the value stored under a key. Wolfram: <c>assoc[k]</c>.
    /// </summary>
    /// <param name="key">Lookup key.</param>
    /// <remarks>
    /// O(w + c) where w ≤ 7 is the hash-trie depth and c the equal-hash collision-bucket scan;
    /// allocates nothing. Wolfram returns <c>Missing["KeyAbsent", k]</c> for an absent key; this
    ...
    /// <summary>
    /// Determines whether a key is present. Wolfram: <c>KeyExistsQ</c>.
    /// </summary>
    /// <param name="key">Lookup key.</param>
    ...
    /// <summary>
    /// Tries to get the value stored under a key. Wolfram: <c>Lookup</c>.
    /// </summary>
    /// <param name="key">Lookup key.</param>
    ...
    /// <summary>
    /// Gets the entry at a zero-based position in association order. Wolfram: <c>assoc[[n]]</c>
    ...
    /// Wolfram: <c>AssociateTo</c> / <c>assoc[k] = v</c>.
    /// </summary>
    ...
    /// <summary>
    /// Returns an association with a key/value pair at the end. Wolfram: <c>Append</c>.
    /// </summary>
    ...
    /// value, matching the kernel-verified Wolfram rule. Returns the same instance when the key
    ...
    /// <summary>
    /// Returns an association with a key/value pair at the front. Wolfram: <c>Prepend</c>.
    /// </summary>
    ...
    /// and value, matching the kernel-verified Wolfram rule. Returns the same instance when the
    ...
    /// <summary>
    /// Returns an association with a pair inserted before a zero-based position. Wolfram:
    ...
    /// <summary>
    /// Returns an association without a key. Wolfram: <c>KeyDrop</c> with one key.
    /// </summary>
    ...
    /// <summary>
    /// Returns an association without any of the specified keys. Wolfram: <c>KeyDrop</c>.
    /// </summary>
    ...
    /// <summary>
    /// Returns the sub-association of the specified keys in the requested order. Wolfram:
    /// <c>KeyTake</c>.
    /// </summary>
    ...
    /// <summary>
    /// Returns an association without the entry at a zero-based position. Wolfram: <c>Delete</c>
    /// (one-based there).
    /// </summary>
    /// <param name="index">Zero-based position.</param>
    ...
    /// <summary>
    /// Returns the contiguous sub-association starting at a zero-based position. Wolfram:
    ...
    /// <summary>
    /// Returns the first <paramref name="count"/> entries. Wolfram: <c>Take[assoc, count]</c>.
    /// </summary>
    ...
    /// <summary>
    /// Returns the association without its first <paramref name="count"/> entries. Wolfram:
    ...
    /// <summary>
    /// Returns the association with entry order reversed. Wolfram: <c>Reverse</c>.
    /// </summary>
    ...
    /// <summary>
    /// Returns the association sorted by key. Wolfram: <c>KeySort</c>.
    /// </summary>
    ...
    /// <summary>
    /// Returns the association sorted by value. Wolfram: <c>Sort</c> (which orders associations by their values).
    /// </summary>
    ...
    /// <summary>
    /// Copies the entries to a new array in association order. Wolfram: <c>Normal</c>.
    /// </summary>
    ...
    private PersistentAssociation<TKey, TValue> SortedBy<TOrder>(Func<Entry, TOrder> selector, IComparer<TOrder> comparer)
    {
        ...
        // Stable sort (Wolfram's Sort/KeySort keep equal entries in association order); the
