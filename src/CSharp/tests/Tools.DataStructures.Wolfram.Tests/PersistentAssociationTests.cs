...
/// mirror the kernel-verified Wolfram Association semantics recorded in the implementation's
/// header comment (verified against Wolfram Engine 14.3 and re-verified against Wolfram 15.0).
/// </summary>
public sealed class PersistentAssociationTests
{
    ...
    /// <summary>Wolfram rule 1: a duplicate key keeps its first position with its last value.</summary>
    [Fact]
    public void Construction_DuplicateKeyKeepsFirstPositionWithLastValue()
    {
        // Wolfram: <|a -> 1, b -> 2, a -> 3|> === <|a -> 3, b -> 2|>.
        AssertOrder(Assoc(("a", 1), ("b", 2), ("a", 3)), ("a", 3), ("b", 2));
    }

    /// <summary>Wolfram rule 3: SetItem updates an existing key in place and appends new keys.</summary>
    [Fact]
    public void SetItem_UpdatesExistingKeyInPlaceAndAppendsNewKeys()
    {
        // Wolfram: assoc["a"] = 5 keeps a's position.
    ...
    /// <summary>Wolfram rule 2: Append moves an existing key to the end.</summary>
    [Fact]
    public void Append_MovesExistingKeyToEnd()
    {
        // Wolfram: Append[<|a -> 1, b -> 2|>, a -> 3] === <|b -> 2, a -> 3|>.
        var assoc = Assoc(("a", 1), ("b", 2));
        AssertOrder(assoc.Append("a", 3), ("b", 2), ("a", 3));
        AssertOrder(assoc.Append("c", 3), ("a", 1), ("b", 2), ("c", 3));
    }

    /// <summary>Wolfram rule 2: Prepend moves an existing key to the front.</summary>
    [Fact]
    public void Prepend_MovesExistingKeyToFront()
    {
        // Wolfram: Prepend[<|a -> 1, b -> 2|>, b -> 3] === <|b -> 3, a -> 1|>.
        var assoc = Assoc(("a", 1), ("b", 2));
        AssertOrder(assoc.Prepend("b", 3), ("b", 3), ("a", 1));
        AssertOrder(assoc.Prepend("c", 3), ("c", 3), ("a", 1), ("b", 2));
    }

    /// <summary>Wolfram rule 6: Join keeps this association's positions with the argument's values.</summary>
    [Fact]
    public void Join_KeepsFirstOperandPositionsWithSecondOperandValues()
    {
        // Wolfram: Join[<|a -> 1, b -> 2|>, <|a -> 3, c -> 4|>] === <|a -> 3, b -> 2, c -> 4|>.
    ...
    /// <summary>Wolfram rule 5: an inserted existing key wins position and value.</summary>
    [Fact]
    public void Insert_ExistingKeyWinsInsertedPositionAndValue()
    {
        // Wolfram: Insert[<|a -> 1, b -> 2, c -> 3|>, a -> 9, 3] === <|b -> 2, a -> 9, c -> 3|>
    ...
    /// <summary>Wolfram rule 4: Take/Drop/GetRange slice by position, keeping keyed lookups consistent.</summary>
    [Fact]
    public void TakeDropAndGetRange_SliceByPosition()
    {
        // Wolfram: Take[<|a -> 1, b -> 2, c -> 3|>, 2] === <|a -> 1, b -> 2|>,
    ...
    /// <summary>Wolfram rule 7: KeySort orders by keys, Sort by values, both stably.</summary>
    [Fact]
    public void KeySortAndSort_OrderStablyAndStayOrdinaryAssociations()
    {
        // Wolfram: KeySort[<|b -> 2, a -> 1|>] === <|a -> 1, b -> 2|>; Sort orders by values.
    ...
    /// <summary>Wolfram rule 8: KeyTake follows the requested key order and skips absent keys.</summary>
    [Fact]
