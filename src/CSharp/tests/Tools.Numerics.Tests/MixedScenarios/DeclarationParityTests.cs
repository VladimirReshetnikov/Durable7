...
public sealed partial class DeclarationParityTests
{
    ...
    private static string ReadNumericsSource(string fileName)
    {
        ...
        {
            string candidate = Path.Combine(directory, "src", "Tools.Numerics", fileName);
        ...
        throw new InvalidOperationException($"Could not locate {fileName} under src/Tools.Numerics.");
    }
