...
| [HAMT](docs/Hamt/overview.md) | Canonical CHAMP map/set with one-descent persistent map factories and one-way owner-token transients; immutable hash bag; strict two-policy persistent bimap; lock-free snapshotting Ctrie; 32/64-bit Patricia maps/sets; and the policy-bound Merkle search tree | [project](src/Tools.DataStructures.Hamt/Tools.DataStructures.Hamt.csproj), [usage](docs/Hamt/usage.md), [API spec](docs/Hamt/api-specification.md), [T2 shipment decision](docs/Hamt/transient-t2-decision.md), [validation](docs/Hamt/validation.md) | `.\test.ps1`; see [tests](tests/Tools.DataStructures.Hamt.Tests/README.md) |
...
Cross-language follow-through is complete: the one-descent HAMT operations, hash bag, strict bimap, neutral
