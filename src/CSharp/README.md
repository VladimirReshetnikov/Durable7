...
| [Wolfram](docs/Wolfram/overview.md) | Wolfram-semantics persistent collections composed from the HAMT and FingerTree families: `PersistentList<T>` and insertion-ordered `PersistentAssociation<TKey, TValue>` (primary client: the Tungsten engine) | [project](src/Tools.DataStructures.Wolfram/Tools.DataStructures.Wolfram.csproj), [usage](docs/Wolfram/usage.md), [API spec](docs/Wolfram/api-specification.md), [validation](docs/Wolfram/validation.md) | `dotnet test .\DataStructures.sln`; see [tests](tests/Tools.DataStructures.Wolfram.Tests/README.md) |
