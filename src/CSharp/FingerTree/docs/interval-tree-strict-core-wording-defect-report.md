...
- `src/CSharp/FingerTree/src/Tools.DataStructures.FingerTree/IntervalTree.cs` formerly contained the phrase:
  "Bounds are amortized for ephemeral use, matching the underlying strict measured tree."
- `src/CSharp/FingerTree/README.md` describes the shared general measured core as holding each deep node's middle subtree
  behind a memoized suspension and computing measures lazily.
- `src/CSharp/FingerTree/docs/api-specification.md` describes `FingerTree<TElement, TMeasure, TMeasureOps>` as a
