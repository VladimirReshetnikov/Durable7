...
- Scope: Local restore, build, test, sample, benchmark, stress, and warning-policy guidance for `src/CSharp/FingerTree`
...
From `src/CSharp/FingerTree`:

```powershell
dotnet restore
dotnet build .\FingerTree.sln
dotnet test .\FingerTree.sln
```

For ordinary behavior changes, `dotnet test .\FingerTree.sln` is the main gate because it restores and
...
```text
src/CSharp/FingerTree> dotnet test .\FingerTree.sln
```
