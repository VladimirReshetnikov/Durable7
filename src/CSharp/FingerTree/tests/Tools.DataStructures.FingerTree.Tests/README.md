...
- Scope: xUnit, CsCheck, sample-smoke, model, and stress tests under `src/CSharp/FingerTree/tests/Tools.DataStructures.FingerTree.Tests`
...
From `src/CSharp/FingerTree`, run the full solution test gate:
...
```powershell
dotnet test .\FingerTree.sln --filter FullyQualifiedName~RopePropertyTests
```
...
dotnet test .\FingerTree.sln --filter FullyQualifiedName~TearableConcurrencyStressTests
Remove-Item Env:\FINGERTREE_STRESS_SECONDS
```

Use the workspace [validation guide](../../docs/validation.md) for restore/build split commands, sample and
