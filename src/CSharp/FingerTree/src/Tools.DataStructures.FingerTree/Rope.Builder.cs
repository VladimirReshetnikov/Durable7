...
{
    /// <summary>
    /// Mutable append-only builder for <see cref="Rope{T}"/>. The builder keeps an immutable frozen prefix plus a
    /// mutable staged tail, so snapshots after appended batches freeze only the new tail.
    /// </summary>
    /// <remarks>
    ...
    {
        private readonly ChunkBuilder<T> _tail = new();
        ...
        /// <summary>
        /// Freezes staged elements into an immutable rope. Repeated calls with no intervening mutation return the
        /// same immutable instance.
        /// </summary>
        /// <returns>An immutable rope containing the staged elements.</returns>
        ...
        private IEnumerator<T> Enumerate(Rope<T> snapshot, int version)
        {
        ...
        private void ThrowIfVersionChanged(int capturedVersion)
        {
            if (_version != capturedVersion)
                throw new InvalidOperationException("The rope builder was modified during enumeration.");
        }
    }
}
