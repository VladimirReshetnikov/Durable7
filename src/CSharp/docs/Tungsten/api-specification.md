# Tungsten Collections API Specification
...
- **Zero-based indexing.** All positional parameters are .NET zero-based; the documented Tungsten
...
| Member | Tungsten | Complexity | Notes |
| --- | --- | --- | --- |
...
| `this[TKey]`, `TryGetValue` | `assoc[k]`, `Lookup` | O(w + c), allocation-free | indexer throws `KeyNotFoundException`; Tungsten would return `Missing["KeyAbsent", k]` |
...
- `First`/`Last`/`GetAt` return key/value pairs; Tungsten's return the bare values.
...
- Key equality is the factory comparer, not structural `SameQ`; an engine supplies a
  structural-hash comparer to recover Tungsten key semantics (including normalizing machine-real
