# Tungsten Collections Usage Guide
...
The classic Tungsten append-in-a-loop trap is linear here along a linear history:
...
// <|a -> 3, b -> 2|>: first-occurrence position, last value (Tungsten rule).
...
Tungsten returns `Missing["KeyAbsent", k]` for absent keys; map `TryGetValue`'s `false` to your
...
```csharp
var pair = assoc.GetAt(0);                  // assoc[[1]] (pair; .Value is the Tungsten part)
