...
- Scope: Local restore, build, test, warning-policy, and coverage guidance for `src/CSharp/Numerics`
...
From `src/CSharp/Numerics`:

```powershell
dotnet restore
dotnet build .\Numerics.sln
dotnet test .\Numerics.sln
```

For ordinary behavior changes, `dotnet test .\Numerics.sln` is the main gate because it restores and builds as
...
- decimal and hexadecimal parsing/formatting across UTF-16 and UTF-8 paths;
...
```text
src/CSharp/Numerics> dotnet test .\Numerics.sln
```
