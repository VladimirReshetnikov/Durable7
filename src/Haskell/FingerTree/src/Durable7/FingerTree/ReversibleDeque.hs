...
makeNodeOrChild [] = error "Durable7.FingerTree.ReversibleDeque.makeNodeOrChild: empty child list"
...
nodes _ = error "Durable7.FingerTree.ReversibleDeque.nodes: fewer than two elements"

treeGetLeaf :: Tree a -> Int -> a
treeGetLeaf Empty _ = error "Durable7.FingerTree.ReversibleDeque.treeGetLeaf: empty tree"
...
getInDigit [] _ = error "Durable7.FingerTree.ReversibleDeque.getInDigit: index outside digit"
...
checkedAdd left right
  | left < 0 || right < 0 =
      error "Durable7.FingerTree.ReversibleDeque: internal negative count"
  | right > maxBound - left =
      error "Durable7.FingerTree.ReversibleDeque: length overflow"
...
splitLast :: [a] -> ([a], a)
splitLast [] = error "Durable7.FingerTree.ReversibleDeque.splitLast: empty list"
