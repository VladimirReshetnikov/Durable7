...
fromListWith algebra values =
  ...
  in case remainder of
       ...
       _ -> error "Durable7.FingerTree.RangeUpdateSequence: internal build remainder"
...
cons value sequence =
  case insertAt 0 value sequence of
    Just result -> result
    Nothing -> error "Durable7.FingerTree.RangeUpdateSequence: internal cons boundary"
...
snoc sequence value =
  case insertAt (count sequence) value sequence of
    Just result -> result
    Nothing -> error "Durable7.FingerTree.RangeUpdateSequence: internal snoc boundary"
...
expectCursorEdit operation Nothing = error ("Durable7.FingerTree.RangeUpdateSequence cursor " ++ operation ++ " failed")
...
buildBalanced algebra lengthValue values =
  ...
  in case afterLeft of
       [] -> error "Durable7.FingerTree.RangeUpdateSequence: truncated build input"
       value : afterValue ->
...
combineNonEmpty :: RangeUpdateAlgebra element measureValue tag
                ...
                -> measureValue
combineNonEmpty _ [] =
  error "Durable7.FingerTree.RangeUpdateSequence: internal empty measure query"
...
expectNode Nothing =
  error "Durable7.FingerTree.RangeUpdateSequence: internal missing node"
...
checkedAdd left right
  | left < 0 || right < 0 =
      error "Durable7.FingerTree.RangeUpdateSequence: internal negative count"
  | right > maxBound - left = lengthOverflow
  | otherwise = left + right

lengthOverflow :: a
lengthOverflow = error "Durable7.FingerTree.RangeUpdateSequence: length overflow"
