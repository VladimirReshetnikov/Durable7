...
fromListWith hashPolicy = foldl (flip insert) (emptyWith hashPolicy)
...
insert value (HashSet values) = HashSet (HashMap.insert value () values)
...
union left right = foldl (flip insert) left (toList right)

intersection :: HashSet a -> HashSet a -> HashSet a
intersection left right = foldl addIfMember (clear left) (toList left)
  where
    addIfMember result value
      | member value right = insert value result
      | otherwise = result

difference :: HashSet a -> HashSet a -> HashSet a
difference left right = foldl (flip delete) left (toList right)
...
isSubsetOf left right = all (`member` right) (toList left)
...
overlaps left right = any (`member` right) (toList left)

setEquals :: HashSet a -> HashSet a -> Bool
setEquals left right = size left == size right && isSubsetOf left right
