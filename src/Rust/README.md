...
| --- | --- | --- | --- |
| [Hamt](Hamt/README.md) | Persistent CHAMP collections, map patches, directed graphs, indexed maps, branching connectivity forest, Patricia, and Merkle port with `Arc` structural sharing | `durable7_hamt::{PersistentHashMap, PersistentMapPatch, PersistentDirectedGraph, PersistentIndexedMap, PersistentAncestralConnectionForest, *}` | `.\test.ps1 -Workspace Hamt` |
| [FingerTree](FingerTree/README.md) | Rust checkpoint for the FingerTree family, payload interval map, chunked bit set, positional/measured/text rope cursors, checkpoint-differential map/vector, ancestry-interval queue and deque, monotone-action heap, contextual rank sequence, and mutable DABA Lite FIFO-window aggregator | `durable7_fingertree::{PersistentIntervalMap, PersistentChunkedBitSet, PersistentDeltaMap, PersistentRunDeltaVector, PersistentMonotoneActionHeap, AncestralSliceQueue, BilateralAncestralDeque, ContextualRankSequence, *}` | `.\test.ps1 -Workspace FingerTree` |
