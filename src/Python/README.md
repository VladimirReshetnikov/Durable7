...
This workspace is the typed Python 3.11+ port of the repository-owned HAMT, FingerTree,
...
- `hamt` contains the real 32-way CHAMP map/set, one-way edit sessions, a lock-coordinated
  snapshotting concurrent facade, signed 32/64-bit Patricia collections, and the authenticated
  Merkle tree with persistence, synchronization, proofs, budgets, and typed merge.
...
```python
from vladimir_reshetnikov.data_structures import PersistentHashMap, TextRope, UInt256
...
The package has no runtime dependencies. The validation suite currently contains 101 tests and has
