...
#ifndef TOOLS_DATA_STRUCTURES_TEST_SUPPORT_HEADLESS_TEST_PROCESS_H
#define TOOLS_DATA_STRUCTURES_TEST_SUPPORT_HEADLESS_TEST_PROCESS_H
...
static void __cdecl tds_test_invalid_parameter_handler(
...
static void tds_configure_headless_test_process(void)
{
#if defined(_WIN32)
    ...
    (void)_set_invalid_parameter_handler(tds_test_invalid_parameter_handler);
#endif
#endif
}

static int tds_is_headless_test_process_configured(void)
{
#if !defined(_WIN32)
    return 1;
#else
    ...
    if (_get_invalid_parameter_handler() != tds_test_invalid_parameter_handler) {
        return 0;
    }
...
static int tds_enter_headless_test_process(void)
{
    tds_configure_headless_test_process();
    if (!tds_is_headless_test_process_configured()) {
