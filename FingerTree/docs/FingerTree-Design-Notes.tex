...
These ideas run end to end in two small console samples under \texttt{samples/}. The \emph{text buffer}
...
--- one finger tree under six guises. Both are smoke-tested, so they cannot silently drift from the library.
Run either with:
...
\begin{itemize}[leftmargin=1.4em, itemsep=1pt]
  ...
  \item \texttt{samples/} --- two runnable console tours (the text buffer and the measures showcase of
    Section~\ref{sec:examples}), each smoke-tested from the suite.
