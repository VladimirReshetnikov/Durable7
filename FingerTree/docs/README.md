...
- [Improvement Proposal: General Measured Finger Tree Enumerator Materializes A List](measured-fingertree-enumerator-allocation-improvement-proposal.md) records an allocation/performance improvement opportunity found during the C++ port: the public general measured tree enumerator currently flattens to a list before yielding, unlike the tuned deque's stack enumerator.
- [Improvement Proposal: Rope FromChunks Immutable Storage](rope-fromchunks-immutable-storage-improvement-proposal.md) records an ownership/documentation improvement opportunity found during the C++ port: `Rope<T>.FromChunks` imports `ReadOnlyMemory<T>` without copying, but the backing storage may still be externally mutable.
