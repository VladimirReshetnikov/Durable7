...
- [Proposal: Mutable Builders For FingerTree Collections](mutable-builder-proposal.md) evaluates `System.Collections.Immutable`-style builders for the repository's data structures and recommends a staged implementation: internal bottom-up packers, generic rope builders, sorted collection builders, then optional transient internals only if benchmarks justify them.
...
- [Defect Report: IntervalTree Documentation Calls The Measured Core Strict](interval-tree-strict-core-wording-defect-report.md) records a narrow XML documentation wording issue found during the C++ port and fixed on 2026-07-01: `IntervalTree<T>` now describes the lazy-memoized measured finger tree correctly.
