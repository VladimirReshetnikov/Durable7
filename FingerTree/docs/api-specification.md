...
- **Representation.** A `FingerTree<Chunk<T>, int, ChunkLengthMeasure<T>>` whose leaves are bounded array chunks (`Chunk<T>` wraps a `ReadOnlyMemory<T>` plus a cached length) and whose measure is the total element count (the monoidal sum of chunk lengths). Chunking gives cache-friendly storage and amortizes the per-element tree overhead; the tree's depth is O(log(n / chunk-size)).
...
- **Operations.** O(1) `Count`/`IsEmpty`/`First`/`Last`; O(log n) `this[index]`/`SetItem`/`Insert`/`RemoveAt`/`Split`/`Slice` (plus a bounded O(chunk-size) array copy at the edited chunk); O(log(min(n, m))) `Concat`; O(n)/O(m) bulk `Create`/`CreateRange`/`FromChunks`/`InsertRange`/`RemoveRange`/`ToArray`/`CopyTo`. `AddFirst`/`AddLast` copy the boundary chunk (an O(chunk-size) constant), so bulk construction is preferred for building. Implements `IReadOnlyList<T>` with a chunk-by-chunk enumerator.
...
- **Generality.** This rope is positional (general over the element type). For an arbitrary monoidal secondary measure — line counts for text, byte weights, custom navigation — compose a product measure on the underlying `FingerTree` directly; a dedicated measured rope is a possible future addition.
