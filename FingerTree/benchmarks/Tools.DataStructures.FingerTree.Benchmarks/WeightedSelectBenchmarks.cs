...
// must walk to the middle), the tree split stays flat while the scan grows linearly with Size.
[MemoryDiagnoser]
public class WeightedSelectBenchmarks
{
    ...
    private FingerTree<int, int, SumMeasure<int>> _tree = null!;
    ...
    public void Setup()
    {
        _weights = Enumerable.Range(1, Size).ToArray();   // strictly positive weights
        _tree = FingerTree<int, int, SumMeasure<int>>.CreateRange(_weights);
        _target = _weights.Sum() / 2;                     // midpoint query: worst case for a linear scan
    }

    // O(log n): descend on the cached running sum.
    [Benchmark(Baseline = true)]
    public int Tree_WeightedSelect() =>
