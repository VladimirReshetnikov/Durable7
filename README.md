...
- [C/FingerTree](C/FingerTree/README.md) is the initial C11 port from the C++ workspace. It provides a strict generic measured-tree core, size-measured deque alias, reversible deque facade, sorted set/multiset/map wrappers, generic priority queue, signed 64-bit interval tree facade, text-rope facade, and CTest validation. The C++ lazy-middle concurrency machinery is documented as follow-up work for the C port.
