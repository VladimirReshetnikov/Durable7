...
- [src/Kotlin/FingerTree](src/Kotlin/FingerTree/README.md) is a Kotlin/JVM port of the FingerTree-family collections and newer sequence/streaming cores. Its immutable measured AVL and RRB substrates provide structurally shared deque, measured sequence, vector, sorted bag/set/map, cached-priority queue, max-high interval tree, positional/measured ropes, and measured text helpers; the separate mutable DABA Lite core maintains FIFO monoid aggregates with worst-case bounded callbacks.
