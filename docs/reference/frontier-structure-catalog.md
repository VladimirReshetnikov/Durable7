...
**C measured/text checkpoint.** `ft_measured_rope_cursor` applies the same explicit ownership and
success-only publication rules to an exact measured-rope root plus gap. Ordered before/after partitions
require no inverse or commutativity, and absolute monotone-prefix search returns the first satisfying gap or a
usable end cursor on a miss. Copy, navigation, positional seek, and snapshot perform O(1) structural work plus
one self-owned policy-context allocation; partitions, copied peeks, point edits, and search are O(log n) plus
bounded chunk work, and range insertion is O(m + log n). The nominal `ft_text_rope_cursor` keeps byte-oriented
...
- the seven research-derived collections promoted out of the former `*.Experimental` namespaces into
  `Durable7.FingerTree`/`Durable7.Hamt` and ported to Rust, C, Haskell, Kotlin, C++, OCaml, and Python: the restricted ancestry-interval
  ...
  from theoretical instantiations rather than collapsing them. Coverage is C#, Rust, C, Haskell,
  Kotlin, C++, OCaml, and Python; only TypeScript remains unported pending a named consumer.
  The Rust, C, Kotlin, C++, and Python ports consolidate the two duplicate C# level-ancestor arenas into one shared backend,
  as the [2026-07-29 review](../reviews/experimental-collections-review-2026-07-29.md) recommended,
  and the Haskell port dispenses with the arena object altogether by making each node its own handle;
- `DabaLite<T, TMonoid>`;
