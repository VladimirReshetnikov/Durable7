...
- Status: Current-state catalog - shipped Axis 1 cores, shipped C# Axis 2 C1/C2/C3/T2, C++/Rust positional cursor checkpoints, cross-language semantic CHAMP sessions, and remaining frontier candidates
...
Editor integration, the C++ and Rust positional semantic cursor checkpoints, the optimized C#
...
path-copying ports in C, C++, Haskell, Kotlin, and Rust, and C++ and Rust now have positional
...
| Version-bound cursor / zipper | 2, 3 | C1 positional cursor, C2 measured/text cursor, and C3 samples implemented in C#; C++/Rust positional semantic checkpoints shipped; C4 consumer-gated | C0 selected the C# readonly-struct zipper-as-version; C++/Rust reuse persistent rope path copying without its complexity claim | C# positional/measured cursors and Tour/Editor integration plus C++/Rust positional facades |
...
**Status (2026-07-13): C# C1, C2, and C3 are shipped; C++ and Rust have positional semantic
checkpoints.**
...
adapters remain consumer-gated. C++ `rope_cursor<T>` and Rust `RopeCursor<T>` preserve the positional
gap, immutable branching, unconditional replacement, and retained-snapshot semantics through
root-sharing snapshot-plus-position facades. They deliberately do not claim the C# zipper
representation or its focus-local complexity; measured/text cursors and the other sibling languages
remain unported. The
...
cadence-sixteen transcripts. The C++ and Rust positional semantic checkpoints add API/behavior parity
without asserting zipper or benchmark parity. Those results do not pre-approve later sequence
...
- the Axis 2 C1 positional `RopeCursor<T>` and C2 measured/text
  `MeasuredRopeCursor<T, TMeasure, TMeasureOps>` in C#, plus the C++ `rope_cursor<T>` and Rust
  `RopeCursor<T>` snapshot-plus-gap positional semantic checkpoints.
...
C++/Rust checkpoints make no zipper or focus-local complexity claim; measured/text cursor parity
