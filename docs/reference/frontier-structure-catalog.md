...
- Status: Current-state catalog - shipped Axis 1 cores, shipped C# Axis 2 C1/C2/C3/T2, Haskell/Kotlin/Rust measured/text plus C++/Haskell/Kotlin/Rust positional cursor checkpoints, cross-language semantic CHAMP sessions, and remaining frontier candidates
...
Editor integration, Haskell's, Kotlin's, and Rust's measured/text semantic cursor checkpoints, the C++, Haskell, Kotlin, and
...
snapshot-plus-gap cursor checkpoints without zipper or performance parity. Haskell, Kotlin, and Rust additionally have
...
| Version-bound cursor / zipper | 2, 3 | C1 positional cursor, C2 measured/text cursor, and C3 samples implemented in C#; Haskell/Kotlin/Rust measured/text and C++/Haskell/Kotlin/Rust positional semantic checkpoints shipped; C4 consumer-gated | C0 selected the C# readonly-struct zipper-as-version; sibling checkpoints reuse persistent rope path copying without its complexity claim | C# positional/measured cursors and Tour/Editor integration, Haskell/Kotlin/Rust measured/text facades, and C++/Haskell/Kotlin/Rust positional facades |
...
**Status (2026-07-14): C# C1, C2, and C3 are shipped; Haskell, Kotlin, and Rust have measured/text
and positional semantic checkpoints; C++ has a positional checkpoint.**
...
through root-sharing snapshot-plus-position facades. Haskell's `MeasuredRopeCursor v a`, Kotlin's
`MeasuredRopeCursor<T, M>`, and Rust's `MeasuredRopeCursor<T, P>` add ordered before/after measures
and absolute prefix search over their existing measured checkpoint cores. Their `TextRopeCursor`
surfaces retain the existing text helpers with `Char`-element Haskell, UTF-16 Kotlin, and Unicode-
scalar Rust positions. They deliberately do not claim the C# zipper representation or its focus-
local complexity; measured/text cursors remain unported in C and C++, and the C positional cursor
...
Haskell/Kotlin/Rust measured/text checkpoints add semantic behavior parity without asserting zipper or
...
- the Axis 2 C1 positional `RopeCursor<T>` and C2 measured/text
  ...
  with Haskell `MeasuredRopeCursor v a`/`TextRopeCursor`, Kotlin
...
cursor parity now spans C#, Haskell, Kotlin, and Rust but remains absent in C and C++. The cursor's C4
