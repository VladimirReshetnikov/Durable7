...
| --- | --- | --- |
| `PersistentOrderedSet<T>` | Independently owned HAMT `element -> stamp` + persistent stamp-ordered sequence | **Shipped in C#** in the neutral Ordered workspace with first-representative retention, explicit positioning, receiver-policy algebra, dual-index invariants, and no Tungsten dependency. Required sibling ports remain pending across C, C++, Haskell, Kotlin, Rust, TypeScript, and Python. |
...
| `PersistentHashBag<T>` | Facade over HAMT `T -> int` + cached `long` total count | **Shipped in C#** with checked positive `int` multiplicities, explicit `DistinctCount`/`TotalCount`, first-representative retention, eager receiver-policy normalization, conventional multiset algebra, and expanded/distinct enumeration. Required ports remain pending across C, C++, Haskell, Kotlin, Rust, TypeScript, and Python after the C#-first proposal sequence. |
