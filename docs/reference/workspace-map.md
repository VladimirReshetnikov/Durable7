...
5. `src/Rust/FingerTree` is a Rust semantic checkpoint for the same public family names; its deque now uses
   structurally shared tree storage, and the workspace documents the remaining lazy-spine asymptotic parity work
   locally.
