...
5. `src/Rust/FingerTree` is a Rust semantic checkpoint for the same public family names; its deque, measured tree,
   and measured rope now use structurally shared storage, and the workspace documents the remaining lazy-spine
   asymptotic parity work locally.
