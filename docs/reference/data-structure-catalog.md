...
family namespaces and ported to Rust, C, Haskell, Kotlin, C++, and OCaml. Coverage is **C#, Rust, C,
Haskell, Kotlin, C++, and OCaml** — only Python and TypeScript remain deliberately unported pending a
named consumer, so absence there is a scheduling decision rather than a gap. Every one of them separates shipped
