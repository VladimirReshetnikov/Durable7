...
the version-bound positional `RopeCursor<T>`: an immutable gap cursor for local movement and edits,
retained branches, and cached canonical snapshots. That cursor surface is C#-only today; measured-
rope and text cursors have not shipped. The Rust checkpoint now uses chunked measured storage for
both positional `Rope<T>` and custom-measured `MeasuredRope<T, P>` and stores `TextRope` content in a
newline-measured rope while its API notes track the remaining lazy-spine parity boundary.
