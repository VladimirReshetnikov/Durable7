...
its two children diverge. Descent is then a prefix test and a bit test ---
...
the hash trie genuinely cannot match --- is \term{structural merge}.
...
The trick is generation stamping. Every indirection node carries the generation
...
replacement safe by publishing a descriptor that readers help complete. A reader
...
slip between ``I read the main node'' and ``the root was replaced''. The C\#
...
input entries. It is narrower than an editing session and its contract says so.
...
changes to it''. It adopts a persistent root in \bigoh{1}, accepts edits under
...
shared or sealed. \code{Persist()} seals the token, publishes without traversing
...
update-shaped API --- is designed, prototyped, and deliberately unshipped.
...
\noindent
Most balanced trees treat all positions alike. A finger tree does not: it keeps
...
that type. Cache the measure of every subtree on its root node, and two things
become cheap:
...
computed in any grouping. Commutativity is \emph{not} required, and the library
...
arithmetic: take five bits of $i$ per level and you are there. No size tables, no
...
all. A \term{relaxed} branch carries a small array of cumulative sizes and is
searched rather than computed. Concatenation introduces relaxed branches only
...
elements. Put a run of elements in one contiguous immutable leaf, and you get
...
conversion, line extraction, newline-style handling.
\end{description}
...
nodes instead of touching nine thousand elements.
...
pending tag. Its children do \emph{not}.
...
monoid and two actions. The single most important detail is the composition
...
a persistent library is in an unusually good position to represent them as such.
...
(Figure~\ref{fig:daba}). Between them they delimit the front, left, right,
...
sorted lower bound. Add a count to the measure --- a product measure --- and the
...
two child spines back together. Expected restructuring is \bigoh{1} and there is
...
function of its contents (Figure~\ref{fig:zip}). Insert the same elements in a
...
\noindent
Insertion order is the order humans expect. It is what a configuration file
...
each subtree, and locating the front entry is a descent. Melding two queues is
...
The measured queue has the same bounds \emph{amortised}. This heap's entire
...
A heap cannot find a key. A map cannot find the minimum. The priority search
...
without looking at a single interval inside it.
...
of its contents. Give every node a digest of what it holds, including the digests
...
the key's \term{layer}. Since each additional zero nibble is a 1-in-16 event,
...
\begin{tikzpicture}[x=1mm,y=1mm]
  ...
  \node[d7label,anchor=west,text width=44mm,align=left] at (94,-10)
    {A key's layer is the number of leading zero nibbles of its SHA-256 digest.
     One block holds each consecutive same-layer run, and its digest covers the
     policy domain, its entries, its subtree count, and its child digests.};
\end{tikzpicture}
\caption{Layer assignment in a Merkle search tree. Because the shape comes from
the hashes, two processes that insert the same entries in different orders build
the same tree and compute the same root digest.}
\label{fig:mst}
\end{figure}
...
A \term{block} is one immutable node's canonical bytes, addressed by its digest.
...
A \term{pack} is a set of blocks prepared for transport. A \emph{complete} pack
...
given only the root digest. The canonical \code{MSP2} format carries three kinds:
...
am missing.} The mechanism is a frontier walk. Compare digests from the root
