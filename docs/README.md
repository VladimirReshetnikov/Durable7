...
- [Benchmark-independent next data structures (2026-07-14)](proposals/benchmark-independent-next-structures-2026-07-14.md) - active C#-first design and execution record after the Axis 1 and shipped Axis 2 tranches: persistent-HAMT single-pass updates and `PersistentHashBag<T>` now ship in C#, TypeScript, and Python; the independently owned neutral `PersistentOrderedSet` has shipped in those three languages with no Tungsten dependency. Ordered passed 62/62 focused tests in both Debug and Release; the full serialized C# Release build completed with 0 warnings and 0 errors and the complete C# gate passed 1,355/1,355 tests. Benchmarks were not run and remain postponed for isolation. The algebra-law-gated persistent AVL `RangeUpdateSequence` is the immediate active C# step; afterward, the remaining sibling-language ports proceed with language-local idioms and serialized validation. Frozen layouts, size tiers, GUID specialization, and other evidence- or consumer-gated work remain parked.
