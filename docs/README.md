...
- [Complexity-parity claim validation (2026-08-07)](reviews/complexity-parity-claim-validation-2026-08-07.md) - an independent test of the campaign's closing claim, which does not hold: the rebuilt cores, the struck census rows, and the byte-identical probe fingerprint all reproduce under direct measurement, but the census surveyed substrates rather than the library, so roughly a hundred divergences survive - 64 confirmed by executed experiment. The load-bearing one is that endpoint *pop* was never deferred in any of the five rebuilt cores, so the amortized-O(1) endpoint bound collapses under persistent replay at a flat Θ(log n) per call, invisible because the probe fingerprint has no pop probe; alongside six more OCaml placeholder substrates the census never opened, the A7 log-squared shape still live in Rust and C after being declared closed, two deque types never rebased onto their workspaces' finger trees, and an unclosed equalize-upward obligation on sorted-set algebra where Haskell answers a disjoint union in 2 comparisons and C# pays n. Carries the surviving work order as a ranked appendix.
