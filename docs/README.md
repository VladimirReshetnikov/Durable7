...
- [Axis 1 new-cores review — round 2 (2026-07-12)](reviews/axis1-new-cores-review-round2-2026-07-12.md) - verifies every finding from the first Axis 1 review was correctly fixed by the remediation pass (both Criticals, incl. the Kotlin Ctrie RDCSS port and the C canonical-set overflow, plus Merkle wire, Patricia combine, RRB height/density, CHAMP validators), plus a fresh six-pass review finding no new Critical/High/Medium defect; fixed this round: the Kotlin CHAMP canonicalization test-coverage gap (vacuous guard) and a Rust RRB failure-model parity divergence.
- [`PersistentHashSet<T>` non-structural set algebra (2026-07-12)](reviews/hamt-set-algebra-nonstructural-2026-07-12.md) - open issue: the HAMT set's `Union`/`Intersect`/`Except`/`SymmetricExcept` are element-wise `O(m)` over `IEnumerable<T>` and cannot exploit structural sharing, unlike the sibling Patricia integer sets; source-grounded statement of proposal A2 Phase 2, with the concrete impact, the intra-family inconsistency, the existing node-layer machinery a fix reuses, and the remediation.
