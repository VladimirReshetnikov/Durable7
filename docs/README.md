...
- [Axis 2 lifecycle and sequence cursors](proposals/axis2-lifecycle-and-sequence-cursors.md) - C#-first contract and rollout plan for a genuine owner-token CHAMP transient, a fixed-layout read-optimized frozen hash tier, and a version-bound rope gap cursor with honest snapshot costs; automatic size and key-type specialization are explicitly deferred.
- [Axis 2, cursor-first alternative (2026-07-13)](proposals/axis2-cursor-first-alternative-2026-07-13.md) - adopts the Axis 2 plan's designs but re-sequences them cursor-first (the one deliverable with named in-repo consumers) and adds one-day fail-fast pre-gates before the two benchmark-contingent tiers (frozen lookup vs the already-fast HAMT; transient vs repeated persistent SetItem), plus a branched-cursor amortization measurement; see the paired review.
...
- [Axis 2 lifecycle and sequence-cursor plan review (2026-07-13)](reviews/axis2-lifecycle-and-cursors-review-2026-07-13.md) - design review of the Axis 2 plan: feasibility verified against the CHAMP/Rope/Ctrie sources, strengths preserved, and findings (sequencing inverts the consumer-driven rule; the frozen tier competes with the repo's own fast HAMT and should pre-gate; branched-cursor amortization ungated; transient win-regime unstated), recommending the cursor-first alternative.
