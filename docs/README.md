...
- [Frontier structure catalog](reference/frontier-structure-catalog.md) - current-state record of shipped Axis 1/selected Axis 2 work, the shipped C# implicit-AVL Range-update core, its pending sibling parity, and remaining candidates beyond composition: new cores, hybrid/adaptive representation tiers, niche-specialized siblings, verdicts, and sequencing.
...
- [Benchmark-independent next data structures (2026-07-14)](proposals/benchmark-independent-next-structures-2026-07-14.md) - C#-first design and execution record: persistent-HAMT single-pass updates, `PersistentHashBag`, and the independently owned neutral `PersistentOrderedSet` ship in C#, TypeScript, and Python; the algebra-law-gated implicit-AVL `RangeUpdateSequence` now ships in C#. Both complete serialized C# Debug and Release solution builds report 0 warnings and 0 errors and both full gates pass 1,417/1,417 tests. Steps 1–3 remain pending in C, C++, Haskell, Kotlin, and Rust; Range remains pending in all seven sibling workspaces, including TypeScript and Python. Benchmarks were not run and remain postponed for an isolated session. Frozen layouts, size tiers, GUID specialization, and other evidence- or consumer-gated work remain parked.
