...
- [Derived structure catalog](reference/derived-structure-catalog.md) - historical verified composition survey with current disposition notes, remaining general candidates, and the distinction between Tungsten application evidence and independently owned general structures.
- [Frontier structure catalog](reference/frontier-structure-catalog.md) - current-state record of shipped Axis 1/selected Axis 2 work and remaining candidates beyond composition: new cores, hybrid/adaptive representation tiers, niche-specialized siblings, verdicts, and sequencing.
...
- [Benchmark-independent next data structures (2026-07-14)](proposals/benchmark-independent-next-structures-2026-07-14.md) - active C#-first execution proposal after the Axis 1 and shipped Axis 2 tranches: persistent-HAMT single-pass updates, `PersistentHashBag<T>`, and the independently owned `Tools.DataStructures.Ordered` `PersistentOrderedSet<T>` are shipped in C#. Ordered passed 62/62 focused tests in both Debug and Release; the full serialized C# Release build completed with 0 warnings and 0 errors and the complete C# gate passed 1,355/1,355 tests. Benchmarks were not run and remain postponed for isolation. An algebra-law-gated persistent AVL `RangeUpdateSequence` is the immediate next C# step; after C# completes, the single-pass HAMT APIs and all three new structures must be ported with language-local idioms and serialized validation to C, C++, Haskell, Kotlin, Rust, TypeScript, and Python, with TypeScript and Python mandatory. Frozen layouts, size tiers, GUID specialization, and other evidence- or consumer-gated work remain parked.
