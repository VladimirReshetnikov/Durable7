...
| Workspace docs | What does this library expose, how is it used, and how is it validated? | `src/<Language>/<Family>/README.md`, `src/<Language>/<Family>/docs/`, `src/CSharp/docs/<Family>/` |
...
```text
Not run: C/C++/Rust/Haskell/Kotlin builds, because this change only updates repository-level Markdown indexes and does not alter commands, source, or workspace contracts.
```
